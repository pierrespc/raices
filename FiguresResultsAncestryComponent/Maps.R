...
for(file in filesPops){
  ...
  if(cluster=="NorthWestEurope"){
    ...
    if(cluster=="NorthAfricaCentralSouthAsia"){
      cluster="NorthAfrica"
    }else{
      if(cluster=="NorthAfricaLevant"){
        cluster="MiddleEast"
      }else{
        cluster=cluster
      }
...
for(file in names(admixed)[-1]){
  if(file=="NorthWestEurope"){
    ...
    if(file=="NorthAfricaCentralSouthAsia"){
      cluster="NorthAfrica"
    }else{
      if(file=="NorthAfricaLevant"){
        cluster="MiddleEast"
      }else{
        cluster=file
      }
...
colorsLEG<-data.frame(matrix(0,0,4))
names(colorsLEG)=c("pop","col","size","porcentage")
lastGroup<-"NONE"
for(i in names(groups)){
  ...
  colorsLEG<-rbind(colorsLEG,cbind(pop=i,
                                   col=c19[[i]],
                                   size=size2,
                                   porcentage=paste(round(scatter19GROUP_PM[,i],digits=4)*100,"%",sep="")))
}
colorsLEG$fill=ifelse(colorsLEG$size==size1,NA,colorsLEG$col)
colorsLEG$border=ifelse(colorsLEG$size==size1,NA,"black")
colorsLEG$col=ifelse(colorsLEG$size==size1,colorsLEG$col,"black")
colorsLEG$hjust=ifelse(colorsLEG$size==size1,"center","inward")
...
p <- ggplot(world, aes(long, lat)) +
  ...
  annotate(geom="text",x=xleft-step*1.5,y=seq(ytop,ybottom,-step)+step/2,label=colorsLEG$porcentage,color=colorsLEG$col,size=as.numeric(colorsLEG$size),hjust = "center",vjust = "center")+  
